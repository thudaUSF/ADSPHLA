...
sampletype = "All" #to change to All, comment the next line and write All (or Blood, Brain)
...
for index, row in ChecklistHLA.iterrows():
    ...
    notAPOE4data = get_data(sampletype,conditions,dfapo1,dfapo2,"MW")

    conditions = list(map(lambda x: x.replace('notAPOE4', 'notAPOE2'), conditions))
    dfapo3,dfapo4 = filterdf(conditions,HLA)
    notAPOE2data = get_data(sampletype,conditions,dfapo3,dfapo4,"MW")

    """if item == "DQB1*02:02":
        save = item.translate(str.maketrans('', '', '*:\'\/')) #Problem with saving file if there's asterisk/semicolon
        dfapo3.to_csv(path + save + sampletype + ".csv", index=False) #Save Cases with no duplicates
        dfapo4.to_csv(path + save + sampletype + "comp.csv", index=False) #Save Cases with no duplicates    
        sys.exit()"""
    ...
    if notAPOE4data[5] < .05:
        APOEresults.append(notAPOE4data)
    if notAPOE2data[5] < .05:
        APOEresults.append(notAPOE2data)
        
Headings = ["Sampletype","Receptor","N - Group", "N - Outgroup","MW p-value", "Better"]
HeadingsAge = ["Sampletype","Receptor","N - Group", "N - Outgroup", "Mean - Group", "Mean - Outgroup", "T-test p-value", "Better"]
HeadingsProp = ["Sampletype","Receptor","N - CaseGroup", "N - CaseOutgroup", "Proportion - Group", "N - Controlgroup", "N - ControlOutgroup", "Proportion - Outgroup", "Z-proportion p-value", "Better"]
HeadingsAPOE = ["Sampletype","Receptor","Condition","N - Group", "N - Outgroup","MW p-value", "Better"]

df_brainresults = pd.DataFrame(brainresults)
df_brainresults.columns = Headings
#df_brainresults = df_brainresults.dropna().sort_values(by=["MW p-value"])
df_ageresults = pd.DataFrame(ageresults)
df_ageresults.columns = HeadingsAge
df_propresults = pd.DataFrame(propresults)
df_propresults.columns = HeadingsProp
df_apoeresults = pd.DataFrame(APOEresults)
df_apoeresults.columns = HeadingsAPOE
with pd.ExcelWriter("HLA_Braak_Age_Prop_APOE_" + sampletype + 'results.xlsx') as writer:
    ...
    df_ageresults.to_excel(writer, sheet_name="Age (T-test)")
    df_propresults.to_excel(writer,sheet_name="Case vs Control (Z-proportion)")
    df_apoeresults.to_excel(writer,sheet_name="APOE results (MW)")